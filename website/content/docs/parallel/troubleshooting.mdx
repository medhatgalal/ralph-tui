...
## Tracker File Conflicts

**Symptom:** Tracker status (beads, JSON) is overwritten or produces a conflict after a parallel run.

**Cause:** Git merge merged a stale version of the tracker file from a worker branch back into the main branch.

**Prevention:** Ralph includes built-in protection that saves the main tracker state before a merge and restores it afterward. This ensures that only the main process (the ParallelExecutor) can permanently update task statuses.

**Fix:** If you suspect tracker state is inconsistent:
1. Run `ralph-tui doctor` to check tracker health.
2. For beads: `bd list` to verify tasks.
3. If files were corrupted by a bad merge, use `git checkout HEAD -- .beads/` (or your JSON file) to restore from the last clean commit.
